    ...
    def handle(self, *args, **kwargs):
        ...
        if (Customer.objects.filter(is_utc=False).count() == count and count<100):
            ...
            for record in fetched_records:
                ...
                record.is_pst = True
                record.save()
                logger.info(f"Customer {record.id} time in utc format {utc_time} is updated in pst format {record.Time}")
            ...
            for record in fetched_records:
                ...
                record.is_pst=False
                record.save()
                logger.info(f"Customer {record.id} time in pst_format {pst_time} is updated in utc format {record.Time}")
            self.update_count(count)
        elif count ==100:
            count=0
